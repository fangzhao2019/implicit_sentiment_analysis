...
def convert(result,m,n,featureSet):
    ...
    for i in range(len(result)):
        if len(result[i][0])<m:continue#根据模式长度筛选模式
        ...
        senword=contain_feature(senwordSet,pattern)
        if len(senword)>0:continue
        if len(feature)>0:
            k+=1
            if k%1000==0:print(k)
    ...
    wb.save('CandidatePattern_IM-EX_POS.xlsx')

filterWord=u'，。的 ；了、：吧'
featureSet=load_featureSet('../featureSet.xlsx')
senwordSet=load_featureSet(u'../senwordSet.xlsx')#情感词集

commentSet=load_commentSet(u'Im-Ex_dataset.xlsx',filterWord,method='jieba')
